...
<body>
	<div id="container">
		<%--<form action="/JSPServletBegin/02_postMethod.do">--%>
		...
		<%-- !! 중요 !!
          ...
          현재 <%= ctxPath%> 이  /JSPServletBegin 이므로 <%= ctxPath%>/02_postMethod.do 의 뜻은     
          http://localhost:9090/JSPServletBegin/02_postMethod.do 이라는 말이다.        
     	--%>
     	<%-- form 태그에서 submit(전송)을 하면
	       http://localhost:9090/JSPServletBegin/02_postMethod.do 으로 데이터를 전송시킨다.
       ...
       <%-- 
	        ...
	        /JSPServletBegin/02_postMethod.do 은 누가 처리를 해주는지는 배치서술자인 web.xml 에 기술되어져 있어야 한다.
