...
<script type="text/javascript">
  ...
  $(document).ready(function(){ 
     ...
     $("button#btnSend").click( () => {
        ...
        // 2024-05-14 10:00 (오픈 API 에서는 202405141000 이래야 전송된다.)
        
        let reservedate = $("input#reservedate").val();
        reservedate = reservedate.split("-").join("");
        // 배열이라 문자열로 ["2024","05","14"] 로 바뀌고 join 해서 합쳐야한다. => "20240514"
        ...
        // 202405141000
          ...
          if( reservedate == "" || reservetime == "" ) {
             // 문자를 바로 보내기인 경우 (날짜나 시간을 하나라도 안적었다면)
        ...
        $.ajax({
              ...
              type:"get",
              <%-- data:{"get파라미터":"값","get파라미터":"값"} 이런 형식으로 하는 대신 let dataObj 객체를 바로 보낸다. --%>
              data:dataObj,
              dataType:"json",
...
<div class="container">
	...
	<c:if test ="${not empty requestScope.mvo}">
		<c:set var ="mobile" value = "${requestScope.mvo.mobile}" />	<%-- 변수 선언 --%>
	...
	<div class="text-center mb-5">
       ...
       <%-- history.back 은 화면 캡쳐기 때문에 이미 삭제되어도 떠 있을 수 있다. --%>
