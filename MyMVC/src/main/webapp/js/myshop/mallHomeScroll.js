$(document).ready(function(){
    ...
    // 즉, 맨처음에는 "스크롤" 하지 않더라도 스크롤한 것 처럼 8개의 HIT상품을 게시해주어야 한다는 말이다.
    displayHIT(start);
    ...
    $(window).scroll(function(){
        ...
        // 웹브라우저창의 높이값(디바이스마다 다르게 표현되는 고정값)   => 문서의 높이에서 웹브라우저창의 높이값을 빼서 사용 
        // console.log("$(window).height() => ", $(window).height() );

        // 아래는 스크롤되어진 스크롤탑의 위치값이 웹브라우저창의 높이만큼 내려갔을 경우를 알아보는 것이다.
        console.log( "$(window).scrollTop() => ", $(window).scrollTop() );
        console.log( "$(document).height() - $(window).height() => ", ( $(document).height() - $(window).height() ) );
        // 스크롤이 모두 내려와서 값이 똑같은 값이 나오면 OK / 그렇지 않을 경우 이 방법을 사용
        ...
        // 다 읽고 맨위로 올라갈 경우 스크롤바 작은 상태 유지상태가 되버린다. 그것의 보안
        if($(window).scrollTop() == 0){     // 다시 처음부터 시작하도록 한다.
            
            $("div#displayHIT").empty();
            $("span#end").empty();          // 더이상 조회할 제품이 없습니다. 글씨 비우기
		    ...
		    */
            ...
            if(start == "1" && json.length == 0) {
                ...
                */   
                ...
                // HIT 상품 결과를 출력하기
                $("div#displayHIT").append(v_html);     // 새로 화면 전환이 아니라 계속 추가할 것이므로 append 사용 [위로 추가하는 것 : prepend]

                // span#countHIT 에 지금까지 출력된 상품의 개수를 누적해서 기록한다.
                $("span#countHIT").text(Number($("span#countHIT").text()) + json.length);     // 현재 태그에 있는 값을 읽어오기 + 읽어온 길이

                // 스크롤을 계속해서 클릭하여 countHIT 값과 totalHITCount 값이 일치하는 경우 
                if($("span#countHIT").text() == $("span#totalHITCount").text()){    // 더보기 클릭하여 맨 하단 까지 보여준 경우
                    $("span#end").html("더이상 조회할 제품이 없습니다.");
                    $("span#countHIT").text("0");               // 초기값으로 변경
                }
